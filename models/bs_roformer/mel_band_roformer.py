...
class MelBandRoformer(Module):
    ...
    def forward(
        ...
        if target_stem_idx is not None and 0 <= target_stem_idx < num_stems:
            if self.use_torch_checkpoint:
                masks = checkpoint(self.mask_estimators[target_stem_idx], x, use_reentrant=False)
            else:
                masks = self.mask_estimators[target_stem_idx](x)
            masks = rearrange(masks, 'b t (f c) -> b 1 f t c', c=2)
            ...
            stft_repr_expanded = repeat(stft_repr, 'b 1 ... -> b 1 ...')
            masks_summed = torch.zeros_like(stft_repr_expanded).scatter_add_(2, scatter_indices, masks)
